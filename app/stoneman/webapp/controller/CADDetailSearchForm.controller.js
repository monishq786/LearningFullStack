sap.ui.define([
  ...
  return Controller.extend("stoneman.controller.CADDetailSearchForm", {
    ...
    refreshScreen: function () {
      ...
      this.byId("pdNo").setValue(null);
      this.byId("buyer").setValue(null);
      this.byId("tech").setValue(null);
      this.byId("merchant").setValue(null);
    ...
    onInit: function () {
      ...
      oRouter.getRoute("RouterNameCADDetailSearchForm").attachMatched(this.onRouteMatched, this)
      ...
      that.setLookupNotEditable();
    },
    setLookupNotEditable: function () {
      var sapUiTA_NO = this.getView().byId('buyer')
      sapUiTA_NO.setValueHelpOnly(true)
      var sapUiTA_NO = this.getView().byId('merchant')
      sapUiTA_NO.setValueHelpOnly(true)
      var sapUiTA_NO = this.getView().byId('tech')
      sapUiTA_NO.setValueHelpOnly(true)
      var sapUiTA_NO = this.getView().byId('cadNo')
      sapUiTA_NO.setValueHelpOnly(true)
    },
    ...
    getListData: function () {
      ...
      WebService.getCadDetailSearchData(body).then(function (response) {
    ...
    onAction: function (oEvent) {
      ...
      var oRouter = UIComponent.getRouterFor(this);
      var sData = aData.RETURNDATA[iIndex].CADDETAILUUID;
      oRouter.navTo("RouterNameCADDetailForm", {
    ...
    onRouterClick: function () {
      this.getRouter().navTo("RouterNameCADDetailForm", {
      ...
      WebService.getAPIForFragment(inputId).then(function (response) {
        ...
        if (response.code == 200) {
          ...
          if (inputId == 'buyer') {
            ...
            oData.columnHeaders =
              [{ label: "CadDetailNo", template: "CadDetailNo" },
              { label: "CadDetailUUID", template: "CadDetailUUID   ", visible: false }];
          }
    ...
    onValueHelpCADNo: function () {
      this.onValueHelpRequest("caddtlNo");
      this.inputId = "caddtlNo";
    },
    ...
    _handleSelectedRow: function (oData) {
      ...
      var sSelectedBuyerCode = oData.BusinessPartner; 
      ...
      else if (this.inputId == "caddtlNo") {
        var sSelectedData = oData.CadDetailNo;
        this.byId("cadNo").setValue(sSelectedData);
        var uuid = oData.CadDetailUUID;
    ...
    onSearch: function () {
      ...
      // //var cadNo = this.byId("cadNo").getSelectedKey();
      // var oModel = this.getView().getModel("cadSearchModel"); // Get the model from the view
      // var cadNo = oModel.getProperty("/cadRequuid");
      ...
      // }
      // var sUrl = undefined;
      // if (buyer != undefined && buyer != null && buyer != "") {//So No
      //   if (sUrl == undefined) {
      //     sUrl = "BuyerName eq " + "'" + buyer + "'";

      //   } else {
      //     sUrl = sUrl + " and BuyerName eq " + "'" + buyer + "'";
      //   }
      // }
      // if (date != undefined && date != null && date != "") {//So No
      //   if (sUrl == undefined) {
      //     sUrl = "CrfReqDate eq " + date;

      //   } else {
      //     sUrl = sUrl + " and CrfReqDate eq " + date;
      //   }
      // }
      // if (status != undefined && status != null && status != "") {//So No
      //   if (sUrl == undefined) {
      //     sUrl = "CrfStatus eq " + "'" + status + "'";

      //   } else {
      //     sUrl = sUrl + " and CrfStatus eq " + "'" + status + "'";
      //   }
      // }
      // if (cadNo != undefined && cadNo != null && cadNo != "") {//So No
      //   if (sUrl == undefined) {
      //     sUrl = "CadDetailUUID eq " + cadNo;

      //   } else {
      //     sUrl = sUrl + " and CadDetailUUID  eq " + cadNo;
      //   }
      // }
      // if (techID != undefined && techID != null && techID != "") {//So No
      //   if (sUrl == undefined) {
      //     sUrl = "TechnoUserId_UserID eq " + techID;

      //   } else {
      //     sUrl = sUrl + " and TechnoUserId_UserID  eq " + techID;
      //   }
      // }
    ...
    fetchFilteredData: function (filterData) {
      if (filterData != undefined) {
        var filterBody = {
          ...
          CADDETAILNO: Number(filterData.CRFREQNO),
          CRFSTATUS: filterData.CRFSTATUS,
          CRFREQDATE: filterData.CRFREQDATE,
          Techno_Guid: filterData.Techno
        };
        WebService.getFilteredTCADDetailSearchListAPI(filterBody).then(function (response) {
          if (response.code == 200) {
            var aData;
            var oModel = that.getView().getModel("cadRequestModel");
            aData = oModel.getData();
    ...
    //   if (sUrl != undefined) {
    //     WebService.getFilteredTCADDetailListAPI(sUrl).then(function (response) {
    //       if (response.code == 200) {
